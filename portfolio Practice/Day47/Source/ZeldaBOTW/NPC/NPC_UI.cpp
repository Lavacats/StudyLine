...
#include "Object/ItemClasClothes.h"
...
void UNPC_UI::BuyItemShop()
{
	...
	if (curItem.ItemPrice <= PlayerInventory->getPlayerMoneyCount()) {
		...
		{
			ClothesClass = AItemClasClothes::StaticClass();
			auto Clothes = GetWorld()->SpawnActor<AItemClasClothes>(ClothesClass, PController->getPlayerCharacter()->GetActorLocation(), FRotator::ZeroRotator);
			switch (curItem.ItemType) {
			case ItemTypeValue::ItemType_Tshirts:
				...
				Clothes->setItemTypeValue(curItem.ItemType);
