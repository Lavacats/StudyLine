...
void UPlayerAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
	...
	auto PlayerController = GetWorld()->GetFirstPlayerController();
	if (PlayerController) {
		IsMove = Cast< APlayerCharacterController>(PlayerController)->getMoveController()->getIsMove();
		IsClimming = Cast< APlayerCharacterController>(PlayerController)->getMoveController()->getIsClimming();
		IsBattle = Cast< APlayerCharacterController>(PlayerController)->getBattleController()->getIsBattle();
		IsShoot = Cast< APlayerCharacterController>(PlayerController)->getBattleController()->getIsShoot();
		auto playerState = Cast< APlayerCharacterController>(PlayerController)->getPlayerState();
		if (playerState) {
	...
	if (IsValid(PlayerController))
	{
		...
		auto PlayerState = Cast<APlayerCharacterController>(PlayerController)->getPlayerState();
		if (IsValid(PController) && IsValid(PlayerState)) {
			if (!IsMouseRightButtonDown && PController->getBattleController()->getIsSword() && PlayerState->CheckCurState(E_PlayerState::Player_SWORD) && PController->IsInputKeyDown(EKeys::LeftMouseButton)) {
...
void UPlayerAnimInstance::AnimNotify_StartBow()
{
	...
	if (PlayerState->CheckCurState(E_PlayerState::Player_SHOT)) {
		...
		Cast<APlayerCharacterController>(PlayerController)->getBattleController()->setIsShoot(true);
...
void UPlayerAnimInstance::AnimNotify_StartAIM()
{
	...
	PlayerController->getBattleController()->setCanShootArrow(true);
...
void UPlayerAnimInstance::AnimNotify_DrawSwordEnd()
{
	...
	PlayerState->setPlayerState(E_PlayerState::Player_IDLE);
	Cast<APlayerCharacterController>(PlayerController)->getBattleController()->setIsSword(true);
}
...
void UPlayerAnimInstance::AnimNotify_CounterAttackEnd()
{
	...
	PlayerState->setPlayerState(PlayerState->getPrePlayerState());
	PController->getBattleController()->CounterAttackEnemy();
}
