...
void UPlayerMeshs::SetMeshPosition(float X, float Y, float Z)
{
	rootMesh->SetRelativeLocation(FVector(X, Y, Z));
}

//void UPlayerMeshs::SetWeaponRootComponent(APlayerWeapon * Weapon, const TCHAR * Socket)
//{
//	Weapon->getBow()->SetupAttachment(rootMesh);
//	//Weapon->AttachToComponent(faceMesh, FAttachmentTransformRules::SnapToTargetIncludingScale, Socket);
//	//Weapon = GetWorld()->SpawnActor<UStaticMeshComponent>(FVector::ZeroVector, FRotator::ZeroRotator);
//}

//void UPlayerMeshs::HideCheckPlayerWeapon(UStaticMeshComponent* Weapon, bool checkValue)
//{
//	Weapon->SetVisibility(checkValue);
//}
