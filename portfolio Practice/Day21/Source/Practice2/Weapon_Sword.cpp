...
AWeapon_Sword::AWeapon_Sword()
{
	...
	RootComponent=SwordCollisionComponent;
	SwordMesh->SetupAttachment(SwordCollisionComponent);
	SwordMesh_Sheath->SetupAttachment(SwordCollisionComponent);
...
void AWeapon_Sword::setCharacterEquip(USkeletalMeshComponent * MeshComponent, const TCHAR * Socket)
{
	if (IsValid(SwordCollisionComponent)) {
		SwordCollisionComponent->AttachToComponent(MeshComponent, FAttachmentTransformRules::SnapToTargetIncludingScale, Socket);
	}
}
