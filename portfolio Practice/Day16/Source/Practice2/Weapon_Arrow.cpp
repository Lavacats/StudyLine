...
AWeapon_Arrow::AWeapon_Arrow()
{
	...
	ArrowComponent->SetCollisionProfileName(TEXT("BlockAll"));
...
void AWeapon_Arrow::Tick(float DeltaTime)
{
	...
	if (GetWorld()->LineTraceSingleByChannel(HitResult,StartTrace,EndTrace,ECC_Destructible,CollisionParams))
	{
		if (HitResult.GetActor())
		{
			...
			UDestructibleComponent* Mesh = Cast< UDestructibleComponent>(HitResult.GetActor());
			//ADestructibleActor
			HitResult.GetActor()->GetDestructibleComponent
		}
	}
	else
	{
