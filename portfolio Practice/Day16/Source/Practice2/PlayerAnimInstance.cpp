...
void UPlayerAnimInstance::AnimNotify_StealthSword()
{
	...
	if (PlayerState->CheckCurState(E_PlayerState::Player_SWORD)) {
		...
		PlayerState->setPlayerState(PlayerState->getPrePlayerState());
		Cast<AP2PlayerController>(PlayerController)->getPlayerWeapon()->setWeaponComponent(E_Weapon::Weapon_SWORD, Character->getPlayerMeshComponent()->getRootMesh(), TEXT("LeftUpLegSocket"));
	}
}
