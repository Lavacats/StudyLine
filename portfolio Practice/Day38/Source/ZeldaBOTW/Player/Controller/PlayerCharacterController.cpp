...
void APlayerCharacterController::Tick(float DeltaTime)
{
	...
	KeyDownCheck();
	p2PlayerCharacter->getPlayerUI()->setStaminaVlaue((float)p2PlayerSate->getStaminaPercent());
}
...
{
	if (!p2PlayerCharacter->getPlayerUI()->getInvenrotyActive()) {
...
void APlayerCharacterController::Turn(float NewAxisValue)
{
	...
	if (!p2PlayerCharacter->getPlayerUI()->getInvenrotyActive()) {
		...
		else if (p2PlayerSate->CheckCurState(E_PlayerState::Player_SHIELD)) {
			p2PlayerCharacter->SetActorRotation(FRotator(p2PlayerCharacter->GetActorRotation().Pitch, GetControlRotation().Yaw, p2PlayerCharacter->GetActorRotation().Roll));
		}
		else {
			shotCheck = false;
...
{
	p2PlayerCharacter->getPlayerUI()->ShowInventory();
	//p2PlayerCharacter->getPlayerCamera()->ShakeCameraActor();
}
