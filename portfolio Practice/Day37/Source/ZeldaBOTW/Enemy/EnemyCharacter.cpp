...
void AEnemyCharacter::ShootArrow()
{
	...
	if (Arrow) {
		FVector NewVelocity = GetActorForwardVector() * 1000.0f;
			...
			//Arrow->getMesh()->GetBodyInstance()->bLockXRotation = false;
			//Arrow->getMesh()->GetBodyInstance()->bLockYRotation = false;
			//Arrow->getMesh()->GetBodyInstance()->bLockZRotation = false;
