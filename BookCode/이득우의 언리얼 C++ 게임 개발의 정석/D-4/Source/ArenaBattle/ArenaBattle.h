...
// 새롭게 분류할 ArenaBattle이라는 카테고리를 만듭니다.

#define AB_LOG_CALLINFO (FString(__FUNCTION__) + TEXT("(") + FString::FromInt(__LINE__) + TEXT(")"))
// 로그 정보 함수(FUNCTION)과 라인정보(LINE)을 출력합니다.

#define ABLOG_S(Verbosity) UE_LOG(ArenaBattle,Verbosity,TEXT("%s"),*AB_LOG_CALLINFO)
// 함수와 라인정보(AB_LOG_CALLINFO)와 같은 파일정보를 Verbosity(Warning과 같은 출력레벨)을 받아 출력합니다

#define ABLOG(Verbosity,Format,...)UE_LOG(ArenaBattle,Verbosity,TEXT("%s %s"),*AB_LOG_CALLINFO,*FString::Printf(Format,##__VA_ARGS__))
