	...
	switch (CurrentState) {
			...
			// LOADING 상태는 현재 캐릭터를 읽는 중인 상태입니다.
		{
			...
			else
			{
				...
				// ABGameMode변수에 게임 모드를 불러옵니다
				ABCHECK(nullptr != ABGameMode);
				// 게임 모드가 제대로 불러와졌는지 확인합니다
				int32 TargetLevel = FMath::CeilToInt((float)ABGameMode->GetScore()*0.8f);
				// 현재 점수값을 이용해 현재 레벨을 구합니다
				int32 FinalLevel = FMath::Clamp<int32>(TargetLevel, 1, 20);
				// 현재 점수값에 1을 더합니다 점수값은 1보다 크며 20보다 작아질 수 없습니다
...
float AABCharacter::GetFinalAttackRange() const
{
	...
	// CurrentWeapon이 있다면 AttakRange를 CurrentWeaopon의 GetAttackRange의 값으로 바꾼다.
}
...
float AABCharacter::GetFinalAttackDamage() const
{
	...
	// AttackDamager와 AttackModifier의 값은
	// CurrentWeapon이 있다면, 
	// Attak damager는 CharacterStat의Attack값에 weapon의 AttackDamager값을 더한다,
	// Current  weapon이 없다면 ChracterStat의 값만 더한다
	// AttackModiier의 경우
	// CurrentWeapon이 있다면
	// AttackModifier의 값은 CurrentWeapon의 Modifier의 값이다
	// CurrentWeaopon이 없다면 1의 값을 가진다.
	return AttackDamage* AttackModifier;
	// 최종적으로 주는 값은 서로 곱한 값이다
}
...
void AABCharacter::BeginPlay()
{
	...
	if (bIsPlayer) {
		...
		auto ABPlayerState = Cast<AABPlayerState>(PlayerState);
		ABCHECK(nullptr != ABPlayerState);
		// 플레이어 스테이트 클래스를 가져오고 정상적으로 불러왔는지 확인합니다.
		AssetIndex = ABPlayerState->GetChracterIndex();
		// 에셋 인덱스는 플레이어 스테이ㅌ트의 캐릭터 인덱스를 사용합니다.
	}
	else {
